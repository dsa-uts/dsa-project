...
The platform runs all services and sandboxes on one Kubernetes cluster, replacing the previous two-layer design of docker compose plus a sandbox-only containerd daemon. The Judge creates sandbox Pods directly (`restartPolicy: Never`, gVisor RuntimeClass) in a dedicated namespace and executes each Step via pods/exec. Its ServiceAccount holds a Role scoped to the sandbox namespace only, so it cannot delete itself or touch service Pods. A built-in ValidatingAdmissionPolicy (CEL) restricts sandbox images to the project's GHCR org with mandatory `@sha256:` digests — the enforcement layer for Digest Pinning. This buys declarative resource control, namespace-scoped RBAC, and admission-time image policy that compose cannot express.
...
- The daemon-level separation of the sandbox-only containerd is gone: every workload shares the cluster container runtime. Isolation now rests on gVisor's kernel isolation (RuntimeClass), with namespace boundaries, RBAC, NetworkPolicy, and admission policy covering the management plane.
- Sandbox Pods use no hostPath; topology-independent workspace delivery is defined in [ADR 0008](0008-topology-agnostic-manifests.md) and [ADR 0009](0009-sandbox-workspace-handoff.md). Cluster ownership and development delivery are defined in [ADR 0019](0019-orbstack-development.md).
