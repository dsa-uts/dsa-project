...
* k3s: コンテナ基盤
  ...
  - manifest は Topology-Agnostic Manifests に従いノード構成を仮定しない。デプロイの既定はシングルノード (ADR 0008)
...
* sandbox: gVisor(runsc) RuntimeClass を指定した一時 Pod
  ...
  - Sandbox Workspace / Preset Directory を sandbox Pod へ受け渡す方式は選定中 (ADR 0009)。いずれの方式でも Preset Directory の read-only mount と sandbox の network deny は維持する
...
### 技術選定
- コンテナ基盤: k3s (containerd 内蔵、gVisor RuntimeClass、デプロイ既定はシングルノード)
