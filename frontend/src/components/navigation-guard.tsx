...
export function NavigationGuard({ children }: { children: ReactNode }) {
  ...
  const message = guard.message ?? 'このページを離れますか？'
  ...
  return <GuardContext value={{ setGuard, canLeave: () => !guard.active || (!guard.busy && window.confirm(message)) }}>
    {children}
    {blocker.state === 'blocked' && <Dialog title="ページを離れる" description={guard.busy ? '処理が終わるまでお待ちください。' : message} onClose={() => blocker.reset()} busy={guard.busy}>
